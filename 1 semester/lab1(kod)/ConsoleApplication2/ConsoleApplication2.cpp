...
int main()
{
	...
	if (mode == 1) {
		...
		for (int i = 0; i < c2; i++) { // Выбираем элемент множества B
			c4 = size(c);
			k = 0;
			for (int j = 0; j < c4; j++) { // Выбираем элемент множества С
				if (b[i] == c[j]) {
					k += 1;
				}
			...
			if (k == 0) {
				c.push_back(b[i]); // Записываем выбранный элемент множества B во множество C
			}
		}
		c4 = size(c);
		cout << "D = {";
		for (int i = 0; i < c4 - 1; i++) {
			cout << c[i] << ", "; // Выводим на экран результат операции объединения множеств А и В (множество C)
		}
		cout << c[c4 - 1] << "}. \n ";
